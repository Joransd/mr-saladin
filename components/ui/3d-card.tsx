    ...
    return (
      ...
      >
        <div
          ...
          className="absolute inset-4 grid h-[calc(100%-2rem)] w-[calc(100%-2rem)] grid-rows-[1fr_auto] rounded-xl shadow-lg relative overflow-hidden"
        >
          {/* Image via next/image avec fill pour optimisation automatique */}
          <Image
            src={imageUrl}
            alt={`${title}, ${subtitle}`}
            fill
          ...
          <div className="relative flex flex-col justify-between rounded-xl p-4 text-white">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <motion.h2
                  ...
                  className="text-xl font-sans font-bold leading-tight break-words"
                >
                  {title}
                </motion.h2>
                <motion.p
                  style={{ transform: "translateZ(40px)" }}
                  className="text-sm font-mono font-light text-white/80"
                >
                  {subtitle}
                </motion.p>
              </div>

              {/* Bouton lien — masqué si pas de href */}
              {href && (
                <motion.a
                  href={href}
                  target="_blank"
            ...
            {href && (
              <motion.button
                ...
                style={{ transform: "translateZ(40px)" }}
                className={cn(
                  "w-full rounded-lg py-3 text-center font-sans font-semibold text-white transition-colors",
